...
var ffmpeg = function (/* inputFilepath, settings, callback */) {
	...
	var _ffmpegInfoConfiguration = function (settings) {
		...
		utils.exec([ffmpeg.bin,'-codecs','2>&1'], settings, function (error, stdout, stderr) {
			...
			// Get the codec list
			var codecList = stdout.match(/ (D.{5}|DE.{4}|\.E.{4}) (.*) {1,} (.*)/g);
			// Scan all codec
			for (var i in codecList) {
				// Get the match value
				var match = / (D.{5}|DE.{4}|\.E.{4}) (.*) {1,} (.*)/.exec(codecList[i]);
				...
				if (match) {
					...
					if (scope.length > 0 && scope[0] == 'D')
                                                format.decode.push(extension);
                                        if (scope.length > 0 && scope[1] == 'E')
